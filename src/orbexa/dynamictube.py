...
def ancillary_controller(
    t_p, t_f, nom_state, act_state, A_nom_val, Lambda, alpha, phi, *args, **kwargs
):
    m = kwargs["m"]
    ...
    state_mod = np.array(
        [
            A_nom_val[i + 3] @ r_tilde - Lambda[i] * v_tilde[i] - min_s_phi[i] * K[i]
            for i in range(3)
        ]
    )
    return state_mod
        ...
        for dragAlpha in [minDragAlpha, maxDragAlpha]:
            for dragBeta in [minDragBeta, maxDragBeta]:
                matrices, _, _ = orbital_ellp_undrag(
                    dt, eccentricity=eccentricity, alpha=dragAlpha, beta=dragBeta
                )
                A, _, _, _, _ = matrices
