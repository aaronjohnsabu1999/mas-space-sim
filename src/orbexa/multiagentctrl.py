...
from orbexa.utils import filenameCreator
...
if __name__ == "__main__":
    ...
    def cylinderRadialConstraint(x, r, f):
        return (
            abs(np.sum([r[j] ** 2 for j in range(0, 2)]) - targetLimit["r_T"] ** 2)
            - 0.1**2
        )

    def cylinderAxialConstraint(x, r, f):
        return (
            abs(np.sum([r[j] ** 2 for j in range(2, 3)]) - 0**2)
            - targetLimit["l_T"] ** 2
        )
    ...
    def ellipsoidConstraint(x, r, f):
        ...
        return (
            abs(np.sum([r[j] ** 2 / ellRad[j] ** 2 for j in range(0, 3)]) - 1**2) - 0.01
        )

    numTimes = int(totalTime / Spacecraft.dt)
    T = Target(
        {"name": "Target", "numStates": 3, "initState": th_T0},
    ...
    if targetShape == "cylinder":
        ...
        T.updateGeometry(
            geometryIneqs=[cylinderRadialConstraint, cylinderAxialConstraint]
        )
    elif targetShape == "ellipsoid":
        shapeParams = {
            "ellRadX": targetLimit["r_Tx"],
            ...
            Chaser(
                {
                    ...
                    "numStates": 6,
                    "initState": np.append(
                        [np.random.uniform(-1.0e3, 1.0e3) for i in range(3)],
                        [np.random.uniform(-1.0e1, 1.0e1) for i in range(3)],
                    ),
                }
            )
        )
    ...
    for t in np.arange(0, totalTime, iterTime):
        ...
        if detectionPhase:
            ...
            G = genNeighbors(
                "maxDist",
                [chaser.currState for chaser in C],
                neighborMaxDist,
                idOffset=True,
            )
            for chaser in C:
            ...
            if debug_plots:
                ...
                ax.scatter(
                    T.currState[0], T.currState[1], T.currState[2], label="Target"
                )
                plt.show()
                plt.figure()
                ax = plt.axes(projection="3d")
                ...
                for chaser in C:
                    plt.plot(
                        np.arange(
                            0,
                            len(tAngPosList[chaser.cid]) * Spacecraft.dt,
                            Spacecraft.dt,
                        ),
                        [x[0] for x in tAngPosList[chaser.cid]],
                        label="Chaser " + str(chaser.cid),
                ...
                for chaser in C:
                    plt.plot(
                        np.arange(
                            0,
                            len(tAngVelList[chaser.cid]) * Spacecraft.dt,
                            Spacecraft.dt,
                        ),
                        [x[0] for x in tAngVelList[chaser.cid]],
                        label="Chaser " + str(chaser.cid),
        ...
        if decoupledMode:
            ...
            if decisionPhase:
                ...
                while True:
                    ...
                    for chaser in C:
                        for neighbor in chaser.neighbors:
                            GInfo[neighbor] = chaser.commData(
                                "goal_list", GInfo[neighbor], neighbor
                            )
                            GInfo[chaser.cid] = C[neighbor - 1].commData(
                                "goal_list", GInfo[neighbor], chaser.cid
                            )
    ...
    if sep_all_plots:
        ...
        if orbit_sim:
            ...
            simulate(
                ...
                simY,
                simZ,
                [(agent + 1) for agent in range(len(S) - 1)],
                len(S) - 1,
            )
        ...
        for spacecraft in S:
            ...
            timeSeq = [
                step * Spacecraft.dt
                for step in range(len(indStateHistory[spacecraft.id][0]))
            ]
            ...
            for i in range(spacecraft.numStates):
                ...
                plt.plot(
                    timeSeq,
                    indStateHistory[spacecraft.id][i],
                    c=cmap(96 * i),
                    label=label,
                )
            plt.legend()
