            ...
            u_t.append(
                np.array(
                    [
                        ...
                        * np.exp(
                            (np.random.random() * 2.0 - 1.0) * np.power(j, 1 / 4.0)
                        )
                        for i in range(3)
                    ]
                )
...
def genAdaptorData(rangeParams, orbitParams, *args, **kwargs):
    ...
    m.time = np.linspace(
        0,
        rangeParams["dt"] * (rangeParams["data_range"] - 1),
        rangeParams["data_range"],
    )
    t = m.Var(value=0.0)
    ...
    for i in range(3):
        ...
        eqs.append(
            x_act[i + 3].dt()
            == np.matmul(A_act(t, p.t_p, m=m)[i + 3], x_act)
            + u_act[i]
...
def runAdaptorOp(operation, operIter, dt, t_s, u_t, W, D, pRange, *args, **kwargs):
    ...
    for paramIter in range(len(pRange)):
        p_est.append(
            m.FV(
                value=np.mean(pRange[paramIter]),
    ...
    for i in range(3):
        ...
        eqs.append(
            x_est[i + 3].dt()
            == np.matmul(A_est(t + t_s, p.t_p, m=m)[i + 3], x_est)
            + u_act[i]
            + d_est(t + t_s, p.t_p, m=m)[i + 3]
        )
    if operation == "FSS":
        eqs.append(
            final * (np.sum([(w_f[i] - x_est[i]) ** 2 for i in range(6)]) - D**2) < 0
        )
        eqs.append(
            final * (np.sum([(w_f[i] - x_est[i]) ** 2 for i in range(6)]) + D**2) > 0
        )
        ...
        eqs.append(
            final * (np.sum([(w_f[i] - x_est[i]) ** 2 for i in range(6)])) < 4.0e-5
        )
    eqs = m.Equations(eqs)
    ...
    if operation == "FSS":
        ...
        if flag == True:
            ...
            output = flag, [
                p_est[paramIter].value[-1] for paramIter in range(len(pRange))
            ]
...
def adaptor(initParams, rangeParams, u_t, D, W, *args, **kwargs):
    ...
    pRange = [
        [
            range_lists[paramIter][0][-1],
            range_lists[paramIter][1][-1],
        ]
        for paramIter in range(numParams)
    ]
        ...
        if dataIter % adaptation_range == 0 and dataIter != 0:
            ...
            while operIter < 2 * (len(pRange) - 1):
                adaptorArgs = {
                    ...
                    "u_t": [
                        u_t[i][dataIter - adaptation_range : dataIter] for i in range(3)
                    ],
            ...
            for paramIter in range(len(pRange)):
                pRange[paramIter] = [
                    max(min(pXi[paramIter]), pRange[paramIter][0]),
                    min(max(pXi[paramIter]), pRange[paramIter][1]),
                ]
                estimates = [
                    np.mean(pRange[paramIter]) for paramIter in range(len(pRange))
                ]

            flag, estimates = runAdaptorOp(
                operation="Optimal",
                operIter=1,
                dt=dt,
                t_s=(dataIter - adaptation_range) * dt,
            ...
            if (
                flag == False
                and estimates[0] not in pRange[0]
                and estimates[1] not in pRange[1]
            ):
                pEstim = estimates
                ...
                pRange = [
                    [range_lists[i][0][-1], range_lists[i][1][-1]]
