...
from params              import *
from commons             import genShapeData, genSkewSymMat, calcGlobalOcclusion
from dynamics            import cwhEquations
random.seed(0)
  ...
  def updateState(self, *args, **kwargs):
    if 'newAngularPos' in kwargs and 'newAngularVel' in kwargs:
      ...
      if 'torqueVal' in kwargs and 'torqueType' in kwargs:
        ...
        torqueVal  = [np.array([0.0 for i in range(len(self.angularVelocity))]) for j in range(numSteps+1)]
        torqueType = 'zero'
      
      newState  = intg.solve_ivp(fun          = targetStateUpdateFunc,
                                 y0           = np.array([list(self.currState), list(self.angularVelocity)]).flatten(),
  ...
  def getObservedAngVel(self, *args, **kwargs):
    if len(args) == 0 and len(kwargs) == 0:
      ...
      try:
        ...
        raise ValueError("Target.getObservedState() requires a numeric value for 't'.")
      try:
        angVel = self.angularVelocityHistory[int(t/self.dt)]
      except:
        self.updateState(numSteps = int(t/self.dt)-len(self.angularVelocityHistory)+1)
        angVel = self.angularVelocityHistory[int(t/self.dt)]
...
class Chaser(Spacecraft):
  ...
  def updateState(self, numSteps, *args):
    ...
    A, B, _, _, _  = matrices
    self.inputs.extend([np.zeros(len(self.inputs[0])) for i in range(numSteps-len(self.inputs))])
    for i in range(numSteps):
      self.lastInput = self.inputs[0]
      self.currState = np.add(np.matmul(A(0), self.currState),
                              np.matmul(B,    self.lastInput))
  ...
  def observeTarget(self, target: Target, *args):
    ...
    if np.linalg.norm(TState - self.TState) < epsilon and np.linalg.norm(TAngVel - self.TAngVel) < epsilon and len(self.tNInfo) == len(self.neighbors):
  ...
  def calculateGoals(self):
    ...
    for agent in range(1, numAgents+1):
      constraints.append({'type': 'ineq', 'fun': lambda x:   (np.linalg.norm(x[3*agent-3:3*agent]) - self.goalBounds[0])})
      constraints.append({'type': 'ineq', 'fun': lambda x: - (np.linalg.norm(x[3*agent-3:3*agent]) - self.goalBounds[1])})
    constraints = tuple(constraints)
    
    self.goalLocations = opt.minimize(calcGlobalOcclusion,
                                      x0          = np.array([x0i * (sum(self.goalBounds)/len(self.goalBounds)) / np.linalg.norm(x0i)
                                                                 for x0i in x0]).flatten(),
                                      args        = (w, v, np.array(x0).flatten(), self.goalBounds),
                                      options     = {'disp': False, 'maxiter': 100},
                                      constraints = constraints).x
    self.occlusion = calcGlobalOcclusion(self.goalLocations, w, v, np.array(x0).flatten(), self.goalBounds)
    return {'goalLocations': self.goalLocations, 'occlusion': self.occlusion}
  ...
  def determineGoalInit(self, type, *args):
    if   type == 'pick_prelim_goal':
      ...
      self.taskAllocAgent = args[0](self.id-1, self.currState[:3], self.goalState, self.neighbors)
  ...
  def determineInputs(self, numSteps):
    ...
    self.inputs = f(cwhEquations, numSteps, self.dt,
...
def trajopt_dynamics(system, numSteps, dt, constraints, bounds,
                     solverParams = {"remote": False, "disp": False, "comp_time": False, "no_soln_disp": True},
                     returnStates = False, *args, **kwargs):
  ...
  for agent in range(len(x)/6):
    for i in range(0,3):
      ...
      eqs.append(x_agent[i+3].dt() == np.matmul(A(t + t_s, t_p, m = m), x_agent)[i+3]
                                              +                         u_agent [i+0]
                                              + d(t + t_s, t_p, m = m)          [i+3])
...
def targetStateUpdateFunc(t, state, dt, momInertia, skewSymMat, torqueVal, torqueType, *args, **kwargs):
  angularPos = state[:3]
  angularVel = state[3:]
  if   torqueType == 'zero':
  ...
  stateUpdate.extend(np.add(np.matmul(np.matmul(np.linalg.inv(momInertia), skewSymMat),
...
def torqueFeedback(state, momInertia):
  ...
  torque     = np.subtract(np.matmul (np.matmul  (genSkewSymMat(angularVel), momInertia), angularVel),
                           np.add    (np.multiply(angularVel, 8.0),
                                      np.multiply(angularPos, 4.0)))
  ...
  if runForceOpt:
    ...
    posit_forces      = forceOpt(copy(torque), prev_posit_forces, next(copy(Spacecraft._ids))-1, forceOpt_constants)  
    positions         = np.array([posit_forces[6*agent-6:6*agent-3] for agent in range(1, numChasers+1)])
    forces            = np.array([posit_forces[6*agent-3:6*agent-0] for agent in range(1, numChasers+1)])
    ...
    print('error of sum of torques = ', 100 * np.linalg.norm(sum([np.cross(positions[agent-1],
                                                                          forces   [agent-1])
...
def forceOpt(torque, prev_posit_forces, numChasers, constants):
  ...
  if any(torque):
    ...
    if not constraintsInCost:
      for agent in range(1, numChasers+1):
        constraints.append({'type': 'ineq', 'fun': lambda x: + (np.linalg.norm(x[6*agent-6:6*agent-4]) - rad_T + epsilon)})
        constraints.append({'type': 'ineq', 'fun': lambda x: - (np.linalg.norm(x[6*agent-6:6*agent-4]) - rad_T - epsilon)})
        constraints.append({'type': 'ineq', 'fun': lambda x: + (               x[6*agent-4]            + len_T/2)})
        constraints.append({'type': 'ineq', 'fun': lambda x: - (               x[6*agent-4]            - len_T/2)})
      constraints.append  ({'type': 'ineq', 'fun': lambda x: - (np.linalg.norm(sum([np.cross(x[6*agent-6:6*agent-3],
                                                                                             x[6*agent-3:6*agent-0])
                                                                                      for agent in range(1, numChasers+1)])
                                                                                 - torque) - epsilon)})
    constraints = tuple(constraints)
    
    posit_forces = opt.minimize(forceMinFunc,
                                x0          = posit_forces,
...
def forceMinFunc(posit_forces, torque, numChasers, constants):
  ...
  cost = sum([np.linalg.norm(posit_forces[6*agent-3:6*agent-0])**2 for agent in range(1, numChasers+1)])
  if constraintsInCost:
    for agent in range(1, numChasers+1):
      if (np.linalg.norm(posit_forces[6*agent-6:6*agent-4]) - rad_T) < -epsilon or (np.linalg.norm(posit_forces[6*agent-6:6*agent-4]) - rad_T) > epsilon:
        cost += inf*(np.linalg.norm(posit_forces[6*agent-6:6*agent-4]) - rad_T)**2
      if (               posit_forces[6*agent-4]            + len_T/2) < 0:
        cost += inf*(               posit_forces[6*agent-4]            + len_T/2)
      if (               posit_forces[6*agent-4]            - len_T/2) > 0:
        cost += inf*(               posit_forces[6*agent-4]            - len_T/2)
    if (np.linalg.norm(sum([np.cross(posit_forces[6*agent-6:6*agent-3],
                                    posit_forces[6*agent-3:6*agent-0])
                              for agent in range(1, numChasers+1)])
                        - torque) > epsilon):
      cost += inf*np.linalg.norm(sum([np.cross(posit_forces[6*agent-6:6*agent-3],
                                              posit_forces[6*agent-3:6*agent-0])
                                        for agent in range(1, numChasers+1)])
                                  - torque)**2
  return cost