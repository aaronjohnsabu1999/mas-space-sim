...
from commons import calcLocalOcclusion, calcGlobalOcclusion
from itertools import count
from mpl_toolkits.mplot3d import Axes3D
from scipy import optimize as opt
from taskalloc import genNeighbors
np.random.seed(0)
...
class DistOptAgent():
  ...
  def updateX(self, X, fun = calcLocalOcclusion, *args):
    ...
    self.x = opt.minimize(fun,
                          x0 = self.allx[-1],
                          args = args[0],
                          options = {'disp': False},
                          constraints = ({'type': 'ineq',
                                          'fun' : lambda x: - (np.linalg.norm(np.subtract(self.allx[-1],
                                                                                          x))\
                                                               - 1.0)})).x
...
if __name__ == "__main__":
  ...
  #      np.array([ 7.0,  4.5]),
  #      np.array([ 2.5, -8.5])]
  X = [np.array([np.random.normal(3.0, 4.0),
                 np.random.normal(1.0, 0.5),
                 np.random.normal(2.0, 8.0),])
        for i in range(8)]
  numAgents = len(X)
  numStates = len(X[0])
  W = [np.array([1.0/numAgents
  ...
  for i in range(numAgents):
    ax1.scatter(agents[i].allx[0][0],
                agents[i].allx[0][1],
                label = 'Agent ' + str(i))
    ax1.plot(   [agents[i].allx[j][0] for j in range(len(agents[i].allx))],
                [agents[i].allx[j][1] for j in range(len(agents[i].allx))],
                label = "Agent " + str(i))
    ax2.plot(   list(range(len(agents[i].allx))),
                [agents[i].allx[j][0] for j in range(len(agents[i].allx))],
                label = "Agent " + str(i))
    ax3.scatter(agents[i].allx[-1][0],
                agents[i].allx[-1][1],
                label = "Agent " + str(i))
    ax4.plot(   list(range(len(agents[i].allx))),
                [agents[i].allx[j][1] for j in range(len(agents[i].allx))],
                label = "Agent " + str(i))
  plt.legend()
  plt.show()
  ...
  for i in range(numAgents):
    ax.scatter(agents[i].allx[0][0],
               agents[i].allx[0][1],
               agents[i].allx[0][2],
               label = 'Agent ' + str(i))
    ax.plot(   [agents[i].allx[j][0] for j in range(len(agents[i].allx))],
               [agents[i].allx[j][1] for j in range(len(agents[i].allx))],
